...
interface CategoryDeleteModalProps {
  isOpen: boolean
  categoryName: string
  /** Number of products still assigned to this category. Deletion is blocked when > 0. */
  productCount: number
  onClose: () => void
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl border border-white/[0.08] bg-slate-900 p-6 shadow-2xl shadow-black/50">
        ...
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-rose-400">Eliminar categoria</p>
        <h3 className="mt-2 text-2xl font-bold tracking-tight text-white">Confirmar accion</h3>

        {hasProducts ? (
          <p className="mt-3 text-sm text-slate-400">
            <span className="font-semibold text-white">{categoryName}</span> tiene{' '}
            <span className="font-semibold text-amber-300">
              {productCount} {productCount === 1 ? 'producto asociado' : 'productos asociados'}
            </span>
            . Reasigna o elimina esos productos antes de borrar la categoria.
          </p>
        ) : (
          <p className="mt-3 text-sm text-slate-400">
            Vas a eliminar <span className="font-semibold text-white">{categoryName}</span> de forma
            permanente.
          </p>
        )}
        ...
        {hasProducts ? (
          <p className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-sm text-amber-200">
            No se puede eliminar una categoria mientras tenga productos asociados.
          </p>
        ) : null}
